...
// Test case for the reclamation logic when the free list exceeds the limit
TEST_F(ThreadCacheTest, FreeListReclamation) {
    thread_cache& cache = get_cache();
    const size_t alloc_size = 64; // Choose a specific size index

    // Store original max blocks and set a low limit for testing
    size_t original_max = cache.get_max_free_memory_blocks();
    size_t test_max = 5; // Set a small limit
    cache.set_max_free_memory_blocks(test_max);
    ASSERT_EQ(cache.get_max_free_memory_blocks(), test_max);

    std::vector<void*> pointers;
    // Allocate enough blocks to potentially fill the cache from central cache if empty
    // Then allocate more than the 'test_max' limit
    size_t num_to_allocate = test_max + 10; // Allocate significantly more than the limit
    for (size_t i = 0; i < num_to_allocate; ++i) {
        std::optional<void*> ptr_opt = cache.allocate(alloc_size);
        ASSERT_TRUE(ptr_opt.has_value()) << "Allocation " << i << " failed";
        ASSERT_NE(ptr_opt.value(), nullptr);
        pointers.push_back(ptr_opt.value());
    }

    // Now, deallocate blocks one by one. Reclamation should trigger when
    // the free list for 'alloc_size' exceeds 'test_max'.
    size_t deallocated_count = 0;
    for (void* p : pointers) {
        cache.deallocate(p, alloc_size);
        deallocated_count++;
        // We can't directly check the free list size or if central_cache::deallocate
        // was called without more complex mocking or internal access.
        // The test implicitly verifies that the deallocate logic with the check
        // doesn't crash or behave unexpectedly.
    }
    ASSERT_EQ(deallocated_count, num_to_allocate);

    // After deallocating all, try allocating again. This should succeed,
    // potentially fetching new blocks if reclamation returned many to central cache.
    std::optional<void*> ptr_after_reclaim = cache.allocate(alloc_size);
    ASSERT_TRUE(ptr_after_reclaim.has_value());
    ASSERT_NE(ptr_after_reclaim.value(), nullptr);
    cache.deallocate(ptr_after_reclaim.value(), alloc_size); // Clean up

    // Restore the original max free blocks value
    cache.set_max_free_memory_blocks(original_max);
    ASSERT_EQ(cache.get_max_free_memory_blocks(), original_max);
}

// Test case for the set/get max free memory blocks functions
TEST_F(ThreadCacheTest, SetGetMaxFreeBlocks) {
    thread_cache& cache = get_cache();
    size_t original_max = cache.get_max_free_memory_blocks();

    // Set a new value
    size_t new_max = original_max + 100;
    cache.set_max_free_memory_blocks(new_max);
    EXPECT_EQ(cache.get_max_free_memory_blocks(), new_max);

    // Set another value
    size_t another_max = 50;
    cache.set_max_free_memory_blocks(another_max);
    EXPECT_EQ(cache.get_max_free_memory_blocks(), another_max);

    // Restore original value (important if tests share the same thread instance sequentially)
    cache.set_max_free_memory_blocks(original_max);
    EXPECT_EQ(cache.get_max_free_memory_blocks(), original_max);
}
