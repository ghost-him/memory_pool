...
protected:
    ...
    // Helper accessors for private members of thread_cache
    std::byte*& get_free_cache(size_t index) { return tc->m_free_cache[index]; }
    size_t& get_free_cache_size(size_t index) { return tc->m_free_cache_size[index]; }
    size_t& get_next_allocate_count(size_t index) { return tc->m_next_allocate_count[index]; }
    ...
    void SetUp() override {
        ...
        for (size_t i = 0; i < memory_pool_v2::size_utils::CACHE_LINE_SIZE; ++i) {
            if (get_free_cache(i) != nullptr) {
                ...
                memory_pool_v2::central_cache::get_instance().deallocate(get_free_cache(i), block_size);

                // 清空 thread_cache 内部记录
                get_free_cache(i) = nullptr;
                get_free_cache_size(i) = 0;
            }
             // 重置下一次分配计数器
            get_next_allocate_count(i) = 0; // 确保 compute_allocate_count 从基线开始
        }
    }
...
TEST_F(ThreadCacheTest, DeallocateNullOrSizeZero) {
    ...
    size_t initial_count = get_free_cache_size(index_to_check);
    byte* initial_ptr = get_free_cache(index_to_check);
    ...
    // 验证内部状态没有改变
    EXPECT_EQ(get_free_cache_size(index_to_check), initial_count);
    EXPECT_EQ(get_free_cache(index_to_check), initial_ptr);
...
TEST_F(ThreadCacheTest, AllocateSmallBlockCacheMiss) {
    ...
    // 初始状态检查
    ASSERT_EQ(get_free_cache(index), nullptr);
    ASSERT_EQ(get_free_cache_size(index), 0);
    size_t initial_next_count = get_next_allocate_count(index); // 可能为 0
    ...
    EXPECT_GT(get_free_cache_size(index), 0); // 应该至少有 N-1 个在缓存中
    EXPECT_NE(get_free_cache(index), nullptr); // 链表头不应为空
    EXPECT_GT(get_next_allocate_count(index), initial_next_count); // 下次分配计数应该增加了
...
TEST_F(ThreadCacheTest, AllocateSmallBlockCacheHit) {
    ...
    size_t count_after_alloc1 = get_free_cache_size(index);
    byte* list_head_after_alloc1 = get_free_cache(index);
    ASSERT_GT(count_after_alloc1, 0); // 确认缓存中有东西了

    // 2. 释放刚刚分配的块，它应该回到 free list 的头部
    tc->deallocate(ptr1, alloc_size);
    EXPECT_EQ(get_free_cache_size(index), count_after_alloc1 + 1); // 数量增加 1
    EXPECT_EQ(get_free_cache(index), reinterpret_cast<byte*>(ptr1)); // ptr1 现在是列表头
    ...
    // 验证内部状态
    EXPECT_EQ(get_free_cache_size(index), count_after_alloc1); // 数量恢复
    EXPECT_EQ(get_free_cache(index), list_head_after_alloc1); // 链表头恢复
}
...
TEST_F(ThreadCacheTest, AllocateLargeBlockDirect) {
    ...
    for (size_t i = 0; i < size_utils::CACHE_LINE_SIZE; ++i) {
        ASSERT_EQ(get_free_cache(i), nullptr);
        ASSERT_EQ(get_free_cache_size(i), 0);
    }
    ...
    for (size_t i = 0; i < size_utils::CACHE_LINE_SIZE; ++i) {
        EXPECT_EQ(get_free_cache(i), nullptr);
        EXPECT_EQ(get_free_cache_size(i), 0);
    }
...
TEST_F(ThreadCacheTest, AlignmentTest) {
    ...
    // 分配后，检查对应 *对齐大小* 的列表状态
    size_t count_after_alloc = get_free_cache_size(index);
    ...
    // 释放后，检查对应 *对齐大小* 的列表状态
    EXPECT_EQ(get_free_cache_size(index), count_after_alloc + 1); // 数量应增加
    EXPECT_EQ(get_free_cache(index), reinterpret_cast<byte*>(ptr)); // 指针应在列表头
}
...
TEST_F(ThreadCacheTest, DeallocateTriggersReturnToCentralCache) {
    ...
    size_t count_after_allocs = get_free_cache_size(index);
    byte* list_head_after_allocs = get_free_cache(index);
    size_t initial_next_alloc_count = get_next_allocate_count(index); // 记录触发前的计数
    ...
    for (size_t i = 0; i < trigger_count; ++i) {
        ...
        size_t size_before_dealloc = get_free_cache_size(index);

        tc->deallocate(ptr_to_deallocate, alloc_size);

        size_t size_after_dealloc = get_free_cache_size(index);
        ...
        if ((size_before_dealloc + 1) * aligned_size > max_bytes) {
            ...
            EXPECT_LT(get_next_allocate_count(index), initial_next_alloc_count)
         ...
         initial_next_alloc_count = get_next_allocate_count(index);
    }

     // 最终检查：确认回收确实发生了（如果我们没有在循环中 break）
     // 可以检查最终的 m_free_cache_size[index] 是否小于等于 trigger_count - expected_recycle_count
     ASSERT_LE(get_free_cache_size(index), trigger_count - expected_recycle_count + count_after_allocs)
